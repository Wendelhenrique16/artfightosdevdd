...
function App() {
  ...
  const [pintura, setPintura] = useState(1) // 1: Rascunho, 2: Cagada, 3: Finalizado
  ...
  //REGRA: (Tamanho * Pintura * Personagens) + Fundo
  const total = (tamanho * pintura * personagens) + fundo
      ...
      <div className="bg-slate-800 p-8 rounded-2xl shadow-2xl border border-slate-700 w-full max-w-md">
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col gap-2">
            <label className="text-xs font-bold text-slate-400 uppercase">Qtd de Personagens</label>
            <input 
              type="number" 
              min="1"
              value={personagens}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-2">
              <label className="text-xs font-bold text-slate-400 uppercase">Pintura</label>
              <select value={pintura} onChange={(e) => setPintura(Number(e.target.value))} className="bg-slate-900 border border-slate-700 p-3 rounded-lg">
                <option value={1}>Rascunho (1)</option>
                <option value={2}>Cagada (2)</option>
                <option value={3}>Finalizado (3)</option>
              </select>
            </div>
            <div className="flex flex-col gap-2">
